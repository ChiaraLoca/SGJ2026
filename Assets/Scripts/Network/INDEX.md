...
| [GameStateDTO.cs](GameStateDTO.cs) | struct `GameStateDTO`, `PlayerDTO`, `CommanderDTO` | Snapshot completo e serializzabile: fase, round, attore attivo, azioni, conteggi mazzo/scarti, conferma `ShopFinished`, giocatori, esito, ultima carta con bersagli e ultima pescata con id carte |
...
| [SessionConfig.cs](SessionConfig.cs) | `enum NetworkMode` + `static SessionConfig` | Scelte del menu che sopravvivono al cambio scena: `Mode` (Pve/Hotseat/Online), `RoomCode`, `Player0Commanders`/`Player1Commanders` (`CommanderKind[]`, null = default). Lette dal `NetworkGameManager` per transport e selezione secchioni |
| [PveOpponentController.cs](PveOpponentController.cs) | `PveOpponentController` | Strategia host-authoritative del computer: valuta carte e bersagli, decide Verifica/fine turno, acquista nello shop e produce normali `GameIntent` |
...
| [NetworkGameManager.cs](NetworkGameManager.cs) | `NetworkGameManager : MonoBehaviour` | Orchestratore. Applica le selezioni di `SessionConfig`, espone le API UI `Submit*()` e, solo host, processa intent e broadcast. In PvE mantiene l'umano come attore 0, separa la fine turno dell'AI dall'ultima carta e attende la presentazione completa prima della decisione successiva |
