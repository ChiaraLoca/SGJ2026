...
| [GameStateDTO.cs](GameStateDTO.cs) | struct `GameStateDTO`, `PlayerDTO`, `CommanderDTO` | Snapshot completo e serializzabile: fase, round, attore attivo, azioni rimanenti, giocatori, esito e ultima carta giocata. `CommanderDTO` include `Kind` (identità, per risolvere nome/ritratto in UI) e `SecondaryUnlocked` |
