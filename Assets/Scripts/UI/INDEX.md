...
| [CommanderSelectController.cs](CommanderSelectController.cs) | `CommanderSelectController : MonoBehaviour` (online: `IInRoomCallbacks` sotto `#if`) | Schermata di selezione (scena `CommanderSelectUI`, griglia 3x2 fino a 6 secchioni). Il tocco apre il dettaglio come popup centrale su `Assets/Background/Note.png`; il pulsante alterna selezione/deselezione. **PvE**: l'umano sceglie la propria coppia e il computer riceve due secchioni distinti casuali. **Online**: ciascuno sceglie sul proprio device e l'host raccoglie entrambe le scelte |
...
| [MainMenuController.cs](MainMenuController.cs) | `MainMenuController : MonoBehaviour` | Menu iniziale (scena `MainMenu`). **Giocatore Singolo** (PvE → selezione della coppia umana, poi `SampleUI`), **Online** (codice stanza → `OnlineLauncher`) e apertura del regolamento |
| [RulebookPopup.cs](RulebookPopup.cs) | `RulebookPopup : MonoBehaviour` | Popup prefab del regolamento su `Assets/Background/Note.png`: pagine navigabili con frecce o swipe, indicatore pagina e chiusura. Prefab in `Assets/Prefabs/UI/RulebookPopup.prefab` |
...
| [Editor/MainMenuRulebookSetup.cs](Editor/MainMenuRulebookSetup.cs) | `MainMenuRulebookSetup` | Genera `RulebookPopup.prefab`, aggiunge la voce **Regolamento** al menu e collega il popup alla scena `MainMenu`. Menu editor `4E → Setup Main Menu Rulebook`. |
...
- **Menu iniziale**: la scena `Assets/Scenes/MainMenu.unity` (prima in Build Settings) ospita `MainMenuController` + `OnlineLauncher`. "Giocatore Singolo" carica `CommanderSelectUI`, sorteggia la coppia PvE e avvia `SampleUI`; "Online" connette via Photon per codice stanza; "Regolamento" apre il post-it paginato anche tramite swipe.
- **Fine shop**: il pulsante si disabilita subito al click e resta grigio in base a `PlayerDTO.ShopFinished`, incluso il round-trip online.
