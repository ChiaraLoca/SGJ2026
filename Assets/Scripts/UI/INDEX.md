...
| [CommanderOptionView.cs](CommanderOptionView.cs) | `CommanderOptionView : MonoBehaviour` | Vista prefab di un comandante selezionabile: nome, ritratto, descrizioni passive; `Bind(data, onSelected)`, `SetSelected(bool)`. Prefab in `Assets/Prefabs/UI/CommanderOption.prefab` |
...
- **Menu iniziale**: la scena `Assets/Scenes/MainMenu.unity` (prima in Build Settings) ospita `MainMenuController` + `OnlineLauncher`. "Stesso telefono" carica `CommanderSelect` in hotseat e poi la scena di gioco (`SampleScene`); "Online" connette via Photon per codice stanza e poi `PhotonNetwork.LoadLevel("SampleScene")`.
