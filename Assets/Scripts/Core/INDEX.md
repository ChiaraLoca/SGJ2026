...
| [PhaseManager.cs](PhaseManager.cs) | `PhaseManager` | State machine: `BeginMatch()`, `HandleVerifica()`, `FinishShop()` → conversione Note→Credits, Fase DRAW, avanzamento round, Esame Finale (`ResolveOutcome()`) |
...
| [TurnManager.cs](TurnManager.cs) | `TurnManager` | Fase PLAY: `StartTurn()`, `TryPlayCard()`, `TryPlayVerifica()`, `EndTurn()`, limiti `CardsAllowedThisTurn` |
...
| [GameContext.cs](GameContext.cs) | `GameContext` + interfaccia `IGameChange` | Contesto di risoluzione: stato in lettura, `RegisterChange()`/`CommitChanges()`, `ResolveCommanders(target)`, `ResolvePlayer(target)`, `SelectedTargets` |
| [GameChanges.cs](GameChanges.cs) | `InstantNoteChange`, `AddActiveEffectChange`, `DrawCardsChange` | Implementazioni `IGameChange` (Command): mutano lo stato al commit e pubblicano `NoteChangedEvent` |
