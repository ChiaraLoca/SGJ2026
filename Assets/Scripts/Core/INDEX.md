...
| [TurnManager.cs](TurnManager.cs) | `TurnManager` | Fase PLAY: contatori `CardsPlayedThisTurn`, `CardsAllowedThisTurn`, `RemainingActions`; avvio/fine turno, gioco carte e Verifica (con `card.ActionCost` per carte multi-azione come Studio Notturno). `ActivateCopyNextCard()` imposta il flag che fa re-risolvere la carta successiva (Copiare). |
...
| [GameChangesExtended.cs](GameChangesExtended.cs) | `GrantActionsChange`, `DoubleActionsChange`, `DrawToHandSizeChange`, `DrawAllChange`, `ForceDiscardRandomChange` (esclude `IsVerifica`), `ForceDiscardByTagChange`, `EqualizeNotesChange`, `SwapNotesChange`, `ReturnFromDiscardChange` (+enum `ReturnDestination`, selezione casuale), `AddShieldChange`, `SetImmunityChange`, `BlockVerificaChange`, `MoveVerificaToDeckBottomChange`, `ReturnCardToHandChange`, `ActivateWikipediaInterceptChange`, `SetCopyNextCardChange` | Modifiche avanzate per i nuovi effetti; helper interno `DeckOps.DrawTopToHand()` |
